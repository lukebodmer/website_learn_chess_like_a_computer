...
def get_lichess_user_games(access_token, username):
    response = requests.get(
        ...
        params={
            ...
            "tags": "true",  # Include game metadata
            "clocks": "true",  # Include clock times as JSON field
            "evals": "true",  # Include evaluations as JSON field
            "accuracy": "true",  # Include accuracy percentages
            "opening": "true",  # Include opening information
            "division": "true",  # Include middlegame/endgame markers
...
def opening_analysis(opening_name):
    ...
    if os.path.exists(os.path.join("app/static", analysis_file)):
        return render_template(
            "analysis_display.html",
            analysis_file=analysis_file,
            opening_name=opening_name,
        )
    else:
        return (
            render_template("analysis_not_found.html", opening_name=opening_name),
            404,
        )
...
def analyze_user(username):
    ...
    try:
        ...
        # Save NDJSON data for analysis
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_filename = f"data/{username}_{timestamp}_games.ndjson"
        with open(json_filename, "w", encoding="utf-8") as f:
            f.write(ndjson_data)

        # Count games by counting lines (each line is one game in NDJSON)
        games_count = len(
            [line for line in ndjson_data.strip().split("\n") if line.strip()]
        )
...
def generate_analysis_report(username):
    ...
    # For testing purposes, we'll look for the most recent NDJSON file for this user
    import glob

    json_files = glob.glob(f"data/{username}_*_games.ndjson")
    if not json_files:
        return "No games data found. Please connect your Lichess account first.", 404

    # Use the most recent file
    most_recent_file = max(json_files)
