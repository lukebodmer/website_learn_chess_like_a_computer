...
const LichessDailyPuzzle: React.FC<LichessDailyPuzzleProps> = ({
  size = 320,
  pieceTheme
}) => {
  ...
  useEffect(() => {
    const fetchPuzzle = async () => {
      try {
        const response = await fetch('/api/daily-puzzle/?source=lichess')
        if (!response.ok) {
          throw new Error('Failed to fetch puzzle')
        }

        const data = await response.json()
        if (data.success && data.puzzles['lichess']) {
          const puzzle = data.puzzles['lichess']
...
export default LichessDailyPuzzle
